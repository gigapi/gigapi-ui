...
export const queryAtom = atomWithStorage<string>("gigapi_current_query", "", {
  ...
  setItem: (key, value) => {
    console.log("🔥 QUERY ATOM SET:", { key, value, timestamp: new Date().toISOString() });
...
export const setQueryAtom = atom(
  ...
  (_get, set, query: string) => {
    console.log("🔥 SET QUERY ATOM CALLED:", { query, timestamp: new Date().toISOString() });
    set(queryAtom, query);
  }
);
    ...
    try {
      ...
      if (QueryProcessor.checkForTimeVariables(query)) {
        ...
        if (selectedTimeField && tableSchema) {
          timeFieldDetails = tableSchema.find(col => 
            col.column_name === selectedTimeField || col.columnName === selectedTimeField
          );
        }
      ...
      const result = await gigapiClient.executeQuery(apiUrl, {
      ...
      const parsedResults = gigapiClient.parseNDJSON(result);
      ...
      set(queryHistoryAtom, [historyItem, ...currentHistory.slice(0, 49)]); // Keep last 50
      
      toast.success(`Query executed successfully (${parsedResults.length} rows in ${executionTime}ms)`);
...
export const clearQueryHistoryAtom = atom(
  null,
  (_get, set) => {
...
import { selectedDbAtom, selectedTableAtom, tableSchemaAtom } from "./database-atoms";
