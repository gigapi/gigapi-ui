...
export const ArtifactProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [operations, setOperations] = useState<
    Record<string, ArtifactOperation[]>
  >({});
  ...
  const log = useCallback(
    (artifactId: string, level: LogLevel, message: string, data?: any) => {
  ...
  const startOperation = useCallback(
    (
      ...
      log(artifactId, "info", `Started ${type} operation`, {
        operationId,
        input,
      });
      return operationId;
  ...
  const endOperation = useCallback(
    (operationId: string, output?: any, error?: Error) => {
        ...
        for (const artifactId in updated) {
          ...
          if (opIndex !== -1) {
            ...
            setMetrics((prev) => {
              ...
              artifactMetrics.totalTime =
                (artifactMetrics.queryExecutionTime || 0) +
  ...
  const getArtifactLogs = useCallback(
    (artifactId: string): ArtifactLog[] => {
      const artifactOps = operations[artifactId] || [];
  ...
  const getArtifactOperations = useCallback(
    (artifactId: string): ArtifactOperation[] => {
