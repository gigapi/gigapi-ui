...
import type { ChatArtifact, ChatSession, ChatMessage, ProposalArtifact as ProposalArtifactType } from '@/types/chat.types';
...
export default function ArtifactRendererWrapper({ 
  ...
  const handleProposalApproval = useCallback(async (proposalId: string) => {
    try {
      setIsExecuting(true);
      ...
      const resultArtifact: ChatArtifact = {
        ...
        data: proposal.chart_type ? {
          ...
          fieldMapping: {
            ...
            yField: proposal.y_axes && proposal.y_axes.length > 0 ? proposal.y_axes[0] : undefined,
            colorField: proposal.y_axes && proposal.y_axes.length > 1 ? proposal.y_axes[1] : undefined
          },
          fieldConfig: {
            defaults: {
              unit: "short",
            ...
            if (art.id === proposalId && art.type === 'proposal') {
              return {
                ...
                data: {
                  ...
                  approved: true,
                  auto_execute: true,
                  execution_status: executionResult.success ? 'completed' as const : 'failed' as const,
      ...
      const feedbackMessage = {
        ...
        content: `Query executed successfully! Here's what I found:\n\n${feedback.summary}\n\n${feedback.insights.length > 0 ? `**Key Insights:**\n${feedback.insights.map(i => `• ${i}`).join('\n')}\n\n` : ''}${feedback.recommendations.length > 0 ? `**Recommendations:**\n${feedback.recommendations.map(r => `• ${r}`).join('\n')}\n\n` : ''}${feedback.follow_up_questions.length > 0 ? `**Would you like me to:**\n${feedback.follow_up_questions.map(q => `• ${q}`).join('\n')}` : ''}`,
      ...
      if (executionResult.success) {
        toast.success(`Query executed successfully! Found ${executionResult.row_count || 0} rows.`);
      } else {
  ...
  const handleProposalRejection = useCallback(async (proposalId: string) => {
    try {
      // Add a rejection message to continue the conversation
      const rejectionMessage: ChatMessage = {
        id: `msg_${Date.now()}_user`,
        role: 'user' as const,
        content: "I'd like to try a different approach. Could you suggest an alternative query?",
        timestamp: new Date().toISOString(),
        metadata: {
  ...
  return (
    <ArtifactRenderer
      artifact={enhancedArtifact as any}
      session={session}
    />
  );
}