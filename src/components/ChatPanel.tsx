...
import ConnectionDialog from "@/components/MCPConnectionDialog";
...
export default function ChatPanel({ isOpen /*, onClose */ }: ChatPanelProps) {
  ...
  const handleNewChat = () => {
    ...
    if (newSession) {
      ...
      toast.error(
        "Could not create new chat. Ensure an AI provider is active."
      );
    }
  };
        ...
        connections.length === 0 && ( // Show only if no connections configured
          <div className="mb-4 p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
            ...
            <p className="text-xs text-orange-600 dark:text-orange-300 mt-1">
              Add and activate an AI provider in the Config tab to start
              chatting.
            </p>
          </div>
        )}
        ...
        activeConnection === null && ( // Show if connections exist but none active
          <div className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
            <div className="flex items-center gap-2 text-yellow-800 dark:text-yellow-200">
              <BotOff className="w-4 h-4" />
              <span className="text-sm font-medium">No active AI provider</span>
            </div>
            <p className="text-xs text-yellow-600 dark:text-yellow-300 mt-1">
              Please select an active AI provider in the Config tab.
            </p>
          </div>
        )}
        ...
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-sm mb-4">
            {isConnected
              ...
              : connections.length === 0
              ? "Configure an AI provider first, then start chatting."
              : "Activate an AI provider in Config, then start chatting."}
          </p>
          <Button
          ...
          >
            <Plus className="w-4 h-4 mr-2" />
            {isConnected ? "Start Chatting" : "Configure AI Provider"}
          </Button>
        </div>
      ) : (
    ...
    <div className="p-4">
      ...
      {connections.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <h3 className="font-medium mb-2">No AI providers configured</h3>
          <p className="text-sm mb-4">
            Add an AI provider to start using the assistant.
          </p>
          <Button onClick={() => setShowConnectionDialog(true)} size="sm">
            <Plus className="w-4 h-4 mr-2" />
            Add AI Provider
                  ...
                  <div className="text-xs text-muted-foreground space-y-0.5">
                    ...
                    {connection.lastUsed && (
                      <div>
                        ...
                        {formatRelativeTime(connection.lastUsed)}
                      </div>
                    )}
                  </div>
                </div>
