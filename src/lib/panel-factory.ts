/**
 * Panel Factory - Creates new panels with Grafana-like structure
 */
...
export class PanelFactory {
  ...
  /**
   * Create a new panel with Grafana-like structure
   */
  static createPanel(options: CreatePanelOptions): PanelConfig {
    ...
    const defaultQuery = query || this.generateDefaultQuery(type);
  ...
  /**
   * Generate default query for panel type - completely generic like Grafana
   */
  private static generateDefaultQuery(type: PanelType): string {
    switch (type) {
      case 'timeseries':
      case 'line':
      case 'area':
      case 'scatter':
        ...
        return `SELECT * FROM your_table WHERE $__timeFilter`;
